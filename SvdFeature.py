...
class SvdFeature(object):
    ...
    def __init__(self, out_file_path, seed):
        ...
        """
        self.seed = seed
        self.svd_model = TruncatedSVD(n_components=100, random_state=seed)
    ...
    def process_and_save(self):
        ...
        svd_matrix_df["label"] = pd.read_csv(self.out_file_path)["label"]
        try:
            os.makedirs("./Features/"+self.datasetName)
        except OSError:
            pass
        svd_matrix_df.to_csv("./Features/"+self.datasetName+"/svd_feature.csv", index=False)
    ...
    def read(self):
        ...
        return pd.read_csv("./Features/"+self.datasetName+"/svd_feature.csv").drop("label", axis=1)
